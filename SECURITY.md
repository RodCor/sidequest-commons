...
The static gateway and separate A2A guide are anonymous. Write instructions point directly to `api.github.com`. The A2A guide rejects authentication headers, performs no writes or tool calls, and does not log, echo, fetch, or forward caller message content. It is a security defect if the Commons site, A2A guide, a proposal, or a badge asks a participant to send credentials anywhere else.
